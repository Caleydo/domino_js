...
export class BlockDecorator implements IObjectDecorator {
  ...
  public decorateHeader(container: JQuery): void {
    this.$header = select(container[0]).append('div').attr('class', 'toolbar');

    //addIconVisChooser(<Element>this.$header.node());
    this.$header.append('i').attr('class', 'fa fa-close').on('click', () => {
      this.decoratedObject.destroy();
    });
    this.$header.append('i').attr('class', 'fa fa-arrows').on('mousedown', () => {
    ...
    this.$header.append('i').attr('class', 'fa fa-plus-square').on('click', () => {
      const e = <MouseEvent> d3event;
      e.preventDefault();
      const amount = 1;
    ...
    this.$header.append('i').attr('class', 'fa fa-minus-square').on('click', () => {
      const e = <MouseEvent> d3event;
      e.preventDefault();
      const amount = -1;
      this.decoratedObject.zoom.zoom(amount, amount);
